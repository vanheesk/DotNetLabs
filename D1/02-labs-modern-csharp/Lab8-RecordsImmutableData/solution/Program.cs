// ============================================================
// Lab 8: Applying Records for Immutable Data Models – SOLUTION
...
Console.WriteLine($"  Hashes equal: {addr1.GetHashCode() == addr2.GetHashCode()}");  // True
...
// Different derived types are never equal, even if base properties match
Vehicle vehicleCar = car;
Vehicle vehicleTruck = truck;
Console.WriteLine($"  car == truck (as Vehicle): {vehicleCar == vehicleTruck}");  // False

// With expression on a derived record preserves the derived type
...
static string ClassifyReading(Temperature reading) => reading switch
{
    (_, < 0, _) => "Freezing",
    (_, >= 0 and < 15, _) => "Cold",
...
static string ProcessPayment(Payment payment) => payment switch
{
...
public record Car(string Make, int Year, int Doors) : Vehicle(Make, Year);
public record Truck(string Make, int Year, double PayloadTons) : Vehicle(Make, Year);

public record Temperature(string Location, double Celsius, DateTime MeasuredAt);

public abstract record Payment(decimal Amount);
public record CreditCard(decimal Amount, string CardNumber, string Expiry) : Payment(Amount);
public record BankTransfer(decimal Amount, string IBAN) : Payment(Amount);
public record DigitalWallet(decimal Amount, string Provider) : Payment(Amount);
